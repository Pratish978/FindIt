...
// Coupons Import (Keep your original imports)
...
const Account = () => {
  ...
  const handleToggleStatus = async (id, currentStatus) => {
    ...
    const feedbackMsg = window.prompt("🎉 Reward Unlock! Share your story:");
  ...
  // ✅ LOGIC FIX: Found item + Recovered status = Reward
  const earnedRewards = myItems.filter(item => {
    const type = String(item.itemType || "").toLowerCase().trim();
    const status = String(item.status || "").toLowerCase().trim();
    return type === 'found' && status === 'recovered';
  });
    ...
    <div className="min-h-screen bg-[#f8fafc]">
      ...
      <main className="pt-32 pb-20 px-6 max-w-6xl mx-auto">
        ...
        <div className="mb-12">
          ...
          {earnedRewards.length > 0 ? (
            ...
            <div className="bg-white border-4 border-dashed border-slate-100 rounded-[3rem] p-12 text-center">
              ...
              <p className="font-black italic uppercase text-slate-400 text-xs">Help someone find their item to unlock rewards</p>
            </div>
          )}
        </div>
        ...
        <div className="bg-white rounded-[3rem] p-8 shadow-2xl border border-slate-50">
          <h2 className="text-xl font-black italic mb-8 flex items-center gap-3"><Clock className="text-blue-600" /> Recent Activity</h2>
              ...
              <div key={item._id} className={`p-6 rounded-[2rem] border-2 transition-all ${item.status === 'recovered' ? 'bg-green-50/40 border-green-200' : 'bg-slate-50/50 border-slate-100'}`}>
                <div className="flex flex-col md:flex-row justify-between items-center gap-4">
                  <div className="flex items-center gap-5 w-full md:w-auto">
                    <img src={item.image || "https://via.placeholder.com/100"} className="w-16 h-16 rounded-2xl object-cover" alt="item" />
                    <div>
                  ...
                  <div className="flex items-center gap-3">
                    <button 
                      ...
                      className={`px-8 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest ${item.status === 'recovered' ? 'bg-green-100 text-green-600' : 'bg-slate-900 text-white'}`}
                    >
                      {item.status === 'recovered' ? 'Unlocked' : 'Mark Reunited'}
                    </button>
                    <button onClick={() => handleDelete(item._id)} className="p-3 text-slate-300 hover:text-red-500"><Trash2 size={20} /></button>
                  </div>
                </div>
              </div>
            ))}
