...
import { Trash2, Clock, Loader2, Ticket, Gift, Download, AlertCircle, RefreshCcw } from "lucide-react";
        ...
        <div className="bg-white rounded-[3rem] p-8 shadow-2xl border border-slate-50">
          ...
          <div className="space-y-4">
            {loading ? (
              ...
              <div key={item._id} className={`p-6 rounded-[2rem] border-2 transition-all ${item.status === 'recovered' ? 'bg-green-50 border-green-200' : 'bg-slate-50 border-slate-100'}`}>
                <div className="flex flex-col md:flex-row justify-between items-center gap-4">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    >
                      {actionId === item._id ? <Loader2 size={14} className="animate-spin" /> : (
                        item.status === 'recovered' ? <><RefreshCcw size={14} /> Reopened Case</> : 'Mark Reunited'
                      )}
                    </button>
