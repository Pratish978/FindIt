...
const ReportLostItem = () => {
  ...
  // ✅ Fixed API URL (n3fm - correctly spelled)
  const API_URL = "https://findit-backend-n3fm.onrender.com/api/items/report";
  ...
  const handleSubmit = async (e) => {
    ...
    if (isElectronic && formData.imei.length < 8) { 
      alert("Please enter a valid IMEI/Serial Number (min 8 digits)."); 
      return; 
    }
    ...
    data.append("imei", isElectronic ? String(formData.imei).trim() : "");
    ...
    // ✅ Note: On web, imageFile is already a Blob/File object
    if (imageFile) data.append("image", imageFile);

    try {
      // Warm up fetch for Render cold start
      const response = await fetch(API_URL, { 
        method: 'POST', 
        body: data,
        // Mode 'cors' is default but explicit is better for debugging
        mode: 'cors'
      });
      ...
      if (response.ok) {
        ...
        alert(`❌ Server Error: ${result.message || "Failed to register"}`);
      }
    } catch (error) { 
      console.error("Submission Error:", error);
      alert("❌ Failed to connect to server. Render might be waking up, please try again in 15 seconds."); 
    } finally { 
      setLoading(false); 
    }
  };
      ...
      <main className="pt-32 pb-20 px-6 max-w-5xl mx-auto">
        <div className="text-center mb-12">
          <span className="bg-red-50 text-red-600 px-4 py-1.5 rounded-full text-xs font-black uppercase flex items-center gap-2 w-fit mx-auto animate-pulse">
            <BrainCircuit size={14} /> AI Match Engine Active
          </span>
        ...
        <div className="bg-white rounded-[2.5rem] shadow-2xl border border-slate-100 overflow-hidden flex flex-col md:flex-row">
          ...
          <div className="md:w-1/3 bg-slate-50 p-8 md:p-10 border-r border-slate-100">
            ...
            <p className="mt-6 text-[11px] text-slate-400 font-medium leading-relaxed italic">
              * A clear photo helps our AI analyze the item and match it with found reports.
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="md:w-2/3 p-8 md:p-12 space-y-6">
            ...
            {isElectronic ? (
              <div className="space-y-3 p-4 bg-red-50 rounded-3xl border border-red-100 animate-in fade-in slide-in-from-top-2">
                <label className="text-xs font-black uppercase tracking-widest text-red-600 flex items-center gap-2">
                  <Lock size={14} /> IMEI / Serial Number *
                </label>
                ...
                <p className="text-[9px] text-red-400 font-bold uppercase tracking-tight">Crucial for auto-verification and matching</p>
              </div>
            ) : (
              <div className="space-y-2">
                <label className="text-xs font-black uppercase tracking-widest text-slate-500">Identifying Mark *</label>
                <input type="text" name="specificDetails" value={formData.specificDetails} placeholder="e.g. Blue case, sticker on back" onChange={handleChange} className="w-full p-4 rounded-2xl bg-slate-50 border border-slate-100 focus:bg-white transition-all font-medium" required />
              </div>
            )}

            <div className="space-y-2">
              <label className="text-xs font-black uppercase tracking-widest text-slate-500">Last Seen Location *</label>
              <input type="text" name="location" value={formData.location} placeholder="Campus area, floor, classroom..." onChange={handleChange} className="w-full p-4 rounded-2xl bg-slate-50 border border-slate-100 focus:bg-white transition-all font-medium" required />
            </div>

            <div className="space-y-2">
              <label className="text-xs font-black uppercase tracking-widest text-slate-500">Contact Number (WhatsApp) *</label>
              <input type="text" name="contact" value={formData.contact} maxLength="10" placeholder="10-digit number" onChange={handleChange} className="w-full p-4 bg-slate-50 border rounded-2xl outline-none focus:border-red-500 font-medium transition-all" required />
            </div>
            ...
            <button type="submit" disabled={loading} className="w-full py-5 bg-red-600 text-white font-black uppercase tracking-widest text-sm rounded-2xl hover:bg-slate-900 transition-all disabled:opacity-70 flex items-center justify-center gap-3 shadow-lg shadow-red-200">
              {loading ? <Loader2 size={18} className="animate-spin" /> : "Publish Report"}
            </button>
          </form>
        </div>
      </main>
      ...
      {matchData && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-6 bg-slate-900/90 backdrop-blur-md">
          <div className="bg-white max-w-md w-full rounded-[3rem] p-10 text-center shadow-2xl border-4 border-red-500">
            ...
            <p className="text-slate-500 mb-8">Great news! An item with matching credentials has already been reported as found by: <br/> <span className="font-black text-slate-900 underline">{matchData.matchedEmail}</span></p>
