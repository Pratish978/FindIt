...
import { Camera, UploadCloud, X, BrainCircuit, Lock, Loader2, CheckCircle, Smartphone } from "lucide-react";
...
const ReportLostItem = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    } catch (error) { alert("❌ Server connection error."); } finally { setLoading(false); }
  };
          ...
          <form onSubmit={handleSubmit} className="md:w-2/3 p-8 md:p-12 space-y-6">
            ...
            {isElectronic ? (
              ...
              <div className="space-y-2">
                ...
                <input type="text" name="specificDetails" value={formData.specificDetails} placeholder="e.g. Red sticker on back" onChange={handleChange} className="w-full p-4 rounded-2xl bg-slate-50 border border-slate-100 focus:bg-white transition-all font-medium" required />
              </div>
            )}
            ...
            <div className="space-y-2">
              ...
              <input type="text" name="location" value={formData.location} placeholder="Where exactly?" onChange={handleChange} className="w-full p-4 rounded-2xl bg-slate-50 border border-slate-100 focus:bg-white transition-all font-medium" required />
            </div>
            ...
            <div className="space-y-2">
              ...
              <input type="text" name="contact" value={formData.contact} maxLength="10" placeholder="e.g. 9876543210" onChange={handleChange} className="w-full p-4 bg-slate-50 border rounded-2xl outline-none focus:border-red-500 font-medium" required />
            </div>
      ...
      {matchData && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-6 bg-slate-900/90 backdrop-blur-md">
          <div className="bg-white max-w-md w-full rounded-[3rem] p-10 text-center shadow-2xl border-4 border-red-500">
            ...
            <p className="text-slate-500 mb-8">Matches report by: <span className="font-black text-slate-900 underline">{matchData.matchedEmail}</span></p>
            <button onClick={() => navigate("/all-found")} className="w-full py-5 bg-red-600 text-white rounded-2xl font-black uppercase tracking-widest">Go to Found Vault</button>
          </div>
        </div>
      )}
      <Footer />
