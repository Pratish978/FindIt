...
import { MapPin, X, CheckCircle, Loader2, Lock, Send, Smartphone, ShieldCheck } from "lucide-react";
    ...
    <div className="min-h-screen bg-[#f8fafc] selection:bg-indigo-100">
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
              {filteredItems.map((item) => (
                <div key={item._id} className="group bg-white rounded-[2.5rem] overflow-hidden shadow-sm border border-slate-100 flex flex-col relative transition-all duration-500 hover:shadow-xl hover:-translate-y-2">
                  <div className="h-72 bg-slate-900 flex items-center justify-center relative overflow-hidden">
                    ...
                    <div className="absolute top-6 right-6">
                       <span className="bg-white/10 backdrop-blur-md text-white border border-white/20 px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-tighter">Private Asset</span>
                    </div>
                    <div className="absolute bottom-6 left-1/2 -translate-x-1/2 w-full px-6 z-10">
                       <div className="bg-indigo-600 text-white py-2 rounded-xl text-center text-[9px] font-black uppercase tracking-widest shadow-xl">Verification Required</div>
                    </div>
                  </div>

                  <div className="p-8">
                    <div className="flex justify-between items-center mb-4">
                      <span className="flex items-center gap-1.5 text-slate-400 text-[10px] font-bold uppercase"><MapPin size={14} className="text-indigo-500" /> {item.location}</span>
                      <span className="bg-indigo-50 text-indigo-600 px-3 py-1 rounded-lg text-[9px] font-black uppercase">{item.aiCategory}</span>
                    </div>
                    <h3 className="text-2xl font-black text-slate-800 mb-8 leading-tight group-hover:text-indigo-600 transition-colors">{item.name}</h3>
                    <button onClick={() => setSelectedItem(item)} className="w-full py-4.5 bg-slate-900 text-white rounded-2xl font-black uppercase text-xs flex items-center justify-center gap-2 hover:bg-indigo-600 transition-all duration-300 shadow-xl shadow-slate-200">Claim Ownership</button>
                  </div>
                </div>
              ))}
            </div>
      ...
      {selectedItem && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-slate-900/40 backdrop-blur-xl transition-opacity" onClick={() => setSelectedItem(null)}></div>
          <div className="bg-white w-full max-w-lg rounded-[3rem] p-10 relative shadow-2xl border border-slate-100 z-10">
            <button onClick={() => setSelectedItem(null)} className="absolute top-8 right-8 text-slate-400 hover:text-slate-900 transition-all"><X size={28}/></button>
            {!showSuccess ? (
              <form onSubmit={handleClaimSubmit} className="space-y-6">
                ...
                {(selectedItem.aiCategory?.toLowerCase().includes("phone") || selectedItem.name.toLowerCase().includes("iphone") || selectedItem.aiCategory?.toLowerCase().includes("laptop")) && (
                  <div className="space-y-2">
                    <label className="text-[10px] font-black uppercase text-indigo-600 flex items-center gap-2 tracking-widest"><Smartphone size={14} /> Device Identity (IMEI/Serial)</label>
                    <input required type="text" inputMode="numeric" className="w-full p-5 rounded-2xl font-mono text-sm border-2 border-slate-50 bg-slate-50 focus:bg-white focus:border-indigo-600 transition-all shadow-inner outline-none" placeholder="Enter ID number" value={claimImei} onChange={(e) => handleNumericInput(e.target.value, setClaimImei, 15)} />
                  </div>
                )}

                <div className="space-y-2">
                  <label className="text-[10px] font-black uppercase text-slate-400 tracking-widest">Ownership Evidence</label>
                  <textarea required placeholder="E.g. Unique marks, lockscreen details, case color..." className="w-full p-5 bg-slate-50 border-2 border-slate-50 rounded-2xl h-36 outline-none focus:bg-white focus:border-indigo-600 transition-all shadow-inner resize-none" value={claimDescription} onChange={(e)=>setClaimDescription(e.target.value)} />
                </div>

                <div className="space-y-2">
                  <label className="text-[10px] font-black uppercase text-slate-400 tracking-widest">Your Contact</label>
                  <input required type="text" inputMode="tel" placeholder="10-digit mobile" className="w-full p-5 bg-slate-50 border-2 border-slate-50 rounded-2xl outline-none focus:bg-white focus:border-indigo-600 transition-all shadow-inner" value={ownerContact} onChange={(e) => handleNumericInput(e.target.value, setOwnerContact, 10)} />
                </div>

                <button type="submit" disabled={isSubmitting} className="w-full py-5 bg-indigo-600 text-white rounded-2xl font-black uppercase flex items-center justify-center gap-3 hover:bg-slate-900 transition-all duration-500 shadow-2xl shadow-indigo-200 disabled:opacity-50">
